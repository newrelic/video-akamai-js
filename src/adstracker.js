...
export default class AmpAdsTracker extends nrvideo.VideoTracker {
  ...
  setValues(e) {
    if (e && e.data) {
      if (e.data.metadata) {
        ...
        try {
          this.height =
            e.data.metadata.getHeight() || e.data.metadata.getVastMediaWidth();
        } catch (err) {}
        try {
          this.width =
            e.data.metadata.getWidth() || e.data.metadata.getVastMediaHeight();
        } catch (err) {}
      }
  ...
  registerListeners() {
    if (this.player.ads) {
      ...
      this.player.ads.addEventListener(
        "breakstart",
        this.onBreakStart.bind(this)
      );
  ...
  unregisterListeners() {
    if (this.player.ads) {
      ...
      this.player.ads.removeEventListener(
        "firstquartile",
  ...
  onError(e) {
    this.setValues(e);
    this.sendError({
      errorMessage: e.data.error.getMessage(),
      errorCode: e.data.error.getErrorCode(),
    });
  }
}
