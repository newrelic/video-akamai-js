...
// -- Version 2.6.2013.01.17 for AIS v3.1.16
...
function ais_client(){
	...
	this.baseurl = 'http://idp.securetve.com/rest/1.0/';
	...
	this.cache_time = {"init":30000,"chooser":30000,"authz_query":30000,"identity":30000,"bounce":2000,"echo_response":30000,"vcs":2000,"vcs_list":2000,"resources":30000};	// Default internal JS resopnse cache settings
...
ais_client.prototype.vcsList = function(contentid_array,force) {	//NEW AS OF AIS 3.1.15 11/2012
	this.log("vcsList: "+contentid_array.join(','),true);
	if (contentid_array == undefined || typeof contentid_array != 'object') {
		this.log("vcsList: no contentid_array provided"+(typeof contentid_array != 'object'),true);
	} else if (force == true || this._use_cache('vcs_list',contentid_array) == false) {
...
ais_client.prototype.loginFormat = function(idp_platformid, response_target) {
	return this.baseurl + this.platform_id + "/init/" + idp_platformid + "?responsemethod=redirect&format=jsonp"+((response_target != undefined)?'&responsetarget='+response_target:'');
}

// Added 9/25/2012 
ais_client.prototype.logoutFormat = function(response_target) {
	return this.baseurl + this.platform_id + "/slo/?responsemethod=redirect&format=jsonp"+((response_target != undefined)?'&responsetarget='+response_target:'');
}
...
ais_client.prototype.login = function(platformid) {
	...
	return this.baseurl + this.platform_id + "/init/"+platformid+"?format=jsonp";
}
...
ais_client.prototype._call_ais = function(call,extra,extra2) {
	var url = this.baseurl + this.platform_id;
	if (call == 'echo') {
		url += "/echo/"+extra;
    } else if (call == 'bounce') {
    	url += "/bounce/";
...
ais_client.prototype.scripts = null;
...
ais_client.prototype._use_cache = function(call) {
	...
	if (response_call == 'authz_query' || response_call == 'vcs') {
		...
		response_call += "-"+arguments[1].join('-');
	}
...
ais_client.prototype.__checkLocalStorageSupport = function() {
	try {
		if (window.localStorage == null)
		  return false;
		var key = this.ls_prepend + "test", value = JSON.stringify({"a":"b"});
